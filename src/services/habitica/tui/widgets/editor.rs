...
use ratatui::{
    ...
    widgets::{Block, Borders, Clear, Padding, StatefulWidget, Widget},
};
use std::cmp::max;
    ...
    if is_sub {
        ...
        field.set_block(
            Block::default()
                ...
                .border_style(Style::default().fg(Palette::BG2.into())),
        );
    }
...
impl<'e> EditorState<'e> {
    ...
    pub fn move_date_cursor(&mut self, m: CursorMove) {
        let date = self
            .date_focus
            .get_or_insert(self.task.date.unwrap_or(OffsetDateTime::now_utc()));
        ...
        let (name, notes, mut subtasks, task, mode) = match task_option {
            Some(task) => {
                ...
                let subtasks = if let Some(st) = &task.checklist {
                    st.iter()
                        .map(|sub| build_input_field(vec![sub.text.clone()], true))
                        .collect()
                } else {
                    Vec::new()
                };
...
impl<'e> StatefulWidget for Editor<'e> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        Block::bordered()
            ...
            .render(checklist_area, buf);

        let [cal_area, diff_area] =
            Layout::vertical([Constraint::Length(7), Constraint::Fill(1)]).areas(right_col);
        ...
        state
            .task
            .date
            .map(|d| event_store.add(d.date(), Style::default().bg(border_bg)));
        ...
        Monthly::new(date, event_store)
            .block(
                Block::default()
                    .borders(Borders::BOTTOM)
                    .border_style(Style::default().fg(border_bg)),
            )
            .show_surrounding(Style::default())
            .show_month_header(Style::default())
            .render(cal_area, buf);

        let diff_chunks: Vec<Rect> =
