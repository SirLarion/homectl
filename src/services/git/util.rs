...
pub fn assert_service_installed() -> Result<(), AppError> {
    ...
    if !Path::new(GIT_BASE_PATH).is_dir() {
        Err(AppError::ServiceError(
            "git root directory not found.".into(),
        ))?
    }
...
pub fn clone_mirror_repository(remote_target: String) -> Result<(), AppError> {
    ...
    let Some(target) = remote_target
        ...
        Err(AppError::ServiceError(format!(
            "Invalid remote_target: {remote_target}"
        )))?
    };
...
pub fn push_mirror_repository(opt_target: Option<String>) -> Result<(), AppError> {
    ...
    Command::new("git")
        .args([
            "push",
            "--mirror",
            format!("{}/{}", forge_remote, target).as_str(),
        ])
        .status()?;
    ...
    Command::new("git")
        .args([
            "push",
            "--mirror",
            format!("{}/{}", gh_remote, target).as_str(),
        ])
        .status()?;
